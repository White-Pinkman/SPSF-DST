...
from utils.data_utils import make_slot_meta, domain2id, OP_SET, make_turn_label, postprocessing,slot2id
...
from utils.data_utils import EXPERIMENT_SLOT
import random
import numpy as np
import os
import time
...
def main(args):
    ...
    model = SomDST(model_config, len(op2id),len(slot2id), len(domain2id), op2id['update'])
...
def model_evaluation(model, test_data, tokenizer, slot_meta, epoch, op_code='4',
    ...
    for di, i in enumerate(test_data):
        ...
        slot_labels_id = torch.LongTensor([i.slot_labels_id]).to(device)
        slot_labels_pos = torch.LongTensor([i.slot_labels_pos]).to(device)
        ...
        with torch.no_grad():
            ...
            d, slot,s, g = model(input_ids=input_ids,
        ...
        slot = slot.cpu()
        slot_labels_id = slot_labels_id.cpu()
        preds, out_label = align_predictions(slot,slot_labels_id)
        preds_list += preds
        out_label_list += out_label
        ...
        for p, g in zip(pred_ops, gold_ops):
            ...
            if p == g:
                ...
                fn_dic[g] += 1
                fp_dic[p] += 1
    #slot_acc = accuracy_score(out_label_list, preds_list)
    #slot_precision = precision_score(out_label_list, preds_list)
    #slot_recall = recall_score(out_label_list, preds_list)
    slot_filling_f1 = f1_score(out_label_list, preds_list)
    with open('preds.txt','w',encoding='utf-8') as p,open('labels.txt','w',encoding='utf-8') as l,open('compare.txt','w',encoding='utf-8')as c:
        for p_list,l_list in zip(preds_list,out_label_list):
            p_str = ' '.join(p_list) + '\n'
            l_str = ' '.join(l_list) + '\n'
            p.write(p_str)
            l.write(l_str)
    ...
    print("Epoch %d slot label F1 : " % epoch, slot_filling_f1)
    ...
    scores = {'epoch': epoch, 'joint_acc': joint_acc_score,'slot_filling_f1':slot_filling_f1,
    ...
    for i in range(batch_size):
        for j in range(seq_len):
            if label_ids[i, j] != nn.CrossEntropyLoss().ignore_index:
                out_label_list[i].append(EXPERIMENT_SLOT[label_ids[i][j]])
                preds_list[i].append(EXPERIMENT_SLOT[preds[i][j]])

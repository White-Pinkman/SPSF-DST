...
EXPERIMENT_SLOT =['O', 'B-hotel-pricerange', 'B-hotel-type', 'B-hotel-book-people', 'B-hotel-book-stay', 'B-hotel-book-day', 'B-train-destination', 'B-train-arriveby', 'B-train-departure', 'I-train-departure', 'B-train-day', 'B-train-book-people', 'B-hotel-stars', 'B-hotel-area', 'B-train-leaveat', 'B-attraction-area', 'B-restaurant-food', 'B-attraction-name', 'I-attraction-name', 'I-restaurant-name', 'B-attraction-type', 'B-hospital-department', 'I-hospital-department', 'B-restaurant-area', 'B-restaurant-pricerange', 'B-restaurant-name', 'B-hotel-name', 'I-hotel-name', 'I-taxi-departure', 'B-restaurant-book-people', 'B-restaurant-book-time', 'B-restaurant-book-day', 'B-hotel-parking', 'I-restaurant-food', 'I-train-destination', 'I-attraction-type', 'B-taxi-leaveat', 'B-taxi-destination', 'B-taxi-arriveby', 'I-taxi-destination', 'B-taxi-departure', 'B-hotel-internet', 'I-taxi-leaveat', 'I-hotel-area', 'I-hotel-book-day', 'I-hotel-stars', 'I-hotel-type', 'I-restaurant-pricerange', 'I-train-leaveat', 'I-hotel-pricerange', 'I-hotel-parking', 'I-hotel-book-people', 'I-attraction-area', 'I-hotel-internet', 'I-train-arriveby', 'I-restaurant-area']
O_INDEX = EXPERIMENT_SLOT.index('O')
PAD_INDEX = -100
domain2id = {d: i for i, d in enumerate(EXPERIMENT_DOMAINS)}
slot2id = {d: i for i, d in enumerate(EXPERIMENT_SLOT)}
MAX_UESR_RESPONESE = 42
...
def prepare_dataset(data_path, tokenizer, slot_meta,
    ...
    for dial_dict in dials:
        ...
        for ti, turn in enumerate(dial_dict["dialogue"]):
            ...
            user_slots = turn["transcript_label"]
            ...
            instance = TrainingInstance(dial_dict["dialogue_idx"], turn_domain,user_slots,
...
class TrainingInstance:
    ...
    def make_instance(self, tokenizer, max_seq_length=None,
        ...
        self.user_slots_labels = self.user_slots.split()
        self.slot_labels_id = [slot2id[slot] for slot in self.user_slots_labels]
        self.slot_labels_id = self.slot_labels_id + (MAX_UESR_RESPONESE - len(self.slot_labels_id)) * [-100]

        diag_2 = diag_2 + (MAX_UESR_RESPONESE - len(diag_2)) * ['[PAD]']
        self.slot_labels_pos = [i+len(diag_1)+2 for i in range(MAX_UESR_RESPONESE)]
        ...
        input_ = diag + state
        segment = segment + [1]*len(state) # 修改segment_id
...
class MultiWozDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        #for f in batch:
        #    print(f.slot_labels_id)
        #    print(f.slot_position,len(f.slot_position))
        #import sys
        #sys.exit()

        slot_labels_id = torch.tensor([f.slot_labels_id for f in batch], dtype=torch.long)
        slot_labels_pos = torch.tensor([f.slot_labels_pos for f in batch], dtype=torch.long)
        ...
        return input_ids, input_mask, segment_ids, state_position_ids, op_ids, domain_ids, gen_ids, max_value, max_update, slot_labels_id,slot_labels_pos

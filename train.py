...
from utils.data_utils import make_slot_meta, domain2id, OP_SET, slot2id,make_turn_label, postprocessing
...
def main(args):
    ...
    if n_gpu > 0:
        """
         ...
         适用场景是网络结构固定（不是动态变化的），网络的输入形状
         （包括 batch size，图片大小，输入的通道）是不变的，其实也就是一般情况下都比较适用。
         torch.backends.cudnn.deterministic将这个 flag 置为True的话，
         每次返回的卷积算法将是确定的，即默认算法。如果配合上设置 Torch 的随机种子为固定值的话，
    ...
    """
    get slot_mata(30 slots) list  and ontology dict {slot_lable:value}
    """
    slot_meta, ontology = make_slot_meta(ontology)
    """
    choose slot operate class
    """
    ...
    """
     ...
     generate_id:[[10036,3],[3309,3]]
     generate_y:[["cheap",[Eos]],['hotel','[eos]']]
     goal_state:[['hotel-pricerange-cheap', 'hotel-type-hotel']]
     input_:['[CLS]', '[SEP]', ';', 'am', 'looking',...]
     input_id:[101,102,1025....]
     input_mask:
     mas_leng_lenght:256
    """
    ...
    test_data_raw = prepare_dataset(data_path=args.test_data_path,
                                    ...
                                    max_seq_length=args.max_seq_length,
                                    op_code=args.op_code)
    #  第一部分 读取数据 需要加入输入槽的标签
    #  input_ = ['[CLS]','[SEP]','I','am',...,'[SEP]','[SLOT]','attraction','area','_','[NULL]'...] 190
    #  input_id与上对应 并且长度为最大长度256
    #  input_mask 256 190个1后面全是0
    #  op2id: 转移状态词典
    #  op_ids:[3,3,3,3,1] 长度30 每个槽的状态转移标签
    #  slot_position:[25,30,35,40,45..] 每个[SLOT]标签的位置
    ...
    model = SomDST(model_config, len(op2id),len(slot2id), len(domain2id), op2id['update'], args.exclude_domain)
    ...
    ckpt = torch.load(args.step1_addslot, map_location='cpu')
    ...
    slot_loss_fnc = nn.CrossEntropyLoss(ignore_index=-100) # 单独用一个 ignore_index
    #mark best_epoch
    best_score = {'epoch': 0, 'joint_acc': 0, 'op_acc': 0, 'final_slot_f1': 0,'slot_filling_f1':0}
    #start train
    for epoch in range(args.n_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            domain_ids, gen_ids, max_value, max_update,slot_labels_id,slot_labels_pos = batch
            ...
            domain_scores, slot_scores,state_scores, gen_scores = model(input_ids=input_ids,
            ...
            if step % 100 == 0:
                if args.exclude_domain is not True:
                    print("[%d/%d] [%d/%d] mean_lossf : %.3f, state_loss : %.3f, gen_loss : %.3f, dom_loss : %.3f,loss_slot: %.3f" \
                          % (epoch+1, args.n_epochs, step,
                             len(train_dataloader), np.mean(batch_loss),
                             loss_s.item(), loss_g.item(), loss_d.item(),loss_slot.item()))
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_data", default='train_dials_relabel.json', type=str)
    parser.add_argument("--dev_data", default='dev_dials_relabel.json', type=str)
    parser.add_argument("--test_data", default='test_dials_relabel.json', type=str)
    ...
    parser.add_argument("--step1_addslot", default='assets/model_best.bin', type=str)
